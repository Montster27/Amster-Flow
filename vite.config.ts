...
// Set PREVIEW_HTTP=1 to force HTTP (e.g. for the Claude Preview server).
const useHttps = !process.env.PREVIEW_HTTP
...
export default defineConfig({
  ...
  server: {
    port: 3001,
    host: process.env.PREVIEW_HTTP ? '127.0.0.1' : 'localhost',
    https: useHttps ? {
