-- ============================================================================
-- FIX CRITICAL DATABASE ISSUES - SAFE VERSION v2
-- Only includes changes that are 100% safe and won't break existing functionality
-- Skips tables that don't exist yet (like notifications, project_module_completion)
-- ============================================================================
...
-- ============================================================================
-- SAFE FIX #2: Add Missing Admin Policies (Only for Existing Tables)
-- ============================================================================
...
BEGIN
  ...
  -- Check if project_module_completion table exists
  SELECT EXISTS (
    SELECT FROM information_schema.tables
    WHERE table_schema = 'public'
    AND table_name = 'project_module_completion'
  ...
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  Missing Tables (migrations not yet run):';
  IF NOT v_notifications_exists THEN
    RAISE NOTICE '   ❌ notifications table missing - run add-notifications-table.sql';
  ELSE
    RAISE NOTICE '   ✅ notifications table exists';
  END IF;
  IF NOT v_module_completion_exists THEN
    RAISE NOTICE '   ❌ project_module_completion table missing - run add-module-completion-tracking.sql';
  ELSE
