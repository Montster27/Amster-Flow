            ...
            >
              ...
              {expandedId === interview.id && (
                <div className="border-t border-gray-200 p-4 bg-gray-50 space-y-4">
                  ...
                  {/* Problem Importance */}
                  <div>
                    <h4 className="font-medium text-gray-800 mb-2">Problem Importance:</h4>
                    <div className="bg-white p-3 rounded border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm font-medium text-gray-700">Rating:</span>
                        <span className="text-sm text-purple-700">{interview.problemImportance}/5</span>
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(level => (
                            <span
                              key={level}
                              className={`text-lg ${
                                level <= interview.problemImportance ? 'text-purple-600' : 'text-gray-300'
                              }`}
                            >
                              ★
                            </span>
                  ...
                  <div>
                    <h4 className="font-medium text-gray-800 mb-2">Current Alternatives:</h4>
                    <p className="text-sm text-gray-700 bg-white p-3 rounded border border-gray-200 whitespace-pre-wrap">
                      {interview.currentAlternatives}
                    </p>
                  </div>
                  ...
                  {interview.memorableQuotes.length > 0 && (
                    <div>
                      <h4 className="font-medium text-gray-800 mb-2">Memorable Quotes:</h4>
                      <div className="space-y-2">
                        {interview.memorableQuotes.map((quote, index) => (
                          <div
                            key={index}
                            className="text-sm text-gray-700 bg-blue-50 p-3 rounded border border-blue-200 border-l-4 italic"
                          >
                            "{quote}"
                          </div>
                        ))}
                  ...
                  {interview.surprisingFeedback && (
                    <div>
                      <h4 className="font-medium text-gray-800 mb-2">Surprising Feedback:</h4>
                      <p className="text-sm text-gray-700 bg-yellow-50 p-3 rounded border border-yellow-200 whitespace-pre-wrap">
                        {interview.surprisingFeedback}
                      </p>
                    </div>
                  )}
                  ...
                  {interview.assumptionTags.length > 0 && (
                    <div>
                      <h4 className="font-medium text-gray-800 mb-2">Assumption Tags:</h4>
                      <div className="space-y-2">
                        {interview.assumptionTags.map((tag, index) => (
                          <div
                            key={index}
                            className={`p-3 rounded border ${getValidationEffectColor(tag.validationEffect)}`}
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-sm font-medium">
                                Assumption #{tag.assumptionId.slice(0, 8)}
                              </span>
                              <span className="text-xs px-2 py-1 bg-white rounded border border-current">
                                {tag.validationEffect} ({tag.confidenceChange > 0 ? '+' : ''}{tag.confidenceChange})
                              </span>
                            </div>
                            {tag.quote && (
                              <p className="text-sm italic mt-2">"{tag.quote}"</p>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  {interview.mentorFeedback && (
                    <div>
                      <h4 className="font-medium text-gray-800 mb-2">Mentor Feedback:</h4>
                      <p className="text-sm text-gray-700 bg-green-50 p-3 rounded border border-green-200 whitespace-pre-wrap">
                        {interview.mentorFeedback}
                      </p>
                    </div>
                  )}
